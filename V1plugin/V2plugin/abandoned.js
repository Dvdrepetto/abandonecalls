(function ($) {
    ...
    var AbandonedPlugin = {
        ...
        setupEventHandlers: function () {
            var self = this;
            $(document).off('click.abandoned').on('click.abandoned', '.btn-recover-abandoned, .btn-finalize-abandoned, .btn-copy-number, .btn-show-note, .btn-history, .btn-requeue-abandoned', function (e) {
                e.preventDefault();
                var $button = $(this);
		...
		else if ($button.hasClass('btn-requeue-abandoned')) {
     			   var uniqueid = $button.data('uniqueid');
        		// Usamos un confirm para evitar clicks accidentales
        			if (confirm("¿Estás seguro de que quieres devolver esta llamada a la cola de pendientes? Se añadirá una nota de 'intento sin respuesta'.")) {
            $.ajax({
                url: 'plugins/abandonedcalls/requeue.php',
                type: 'POST',
                data: { uniqueid: uniqueid },
                success: function (response) {
                    if (response === 'OK') {
                        // Refrescamos ambos widgets para que la llamada desaparezca de "procesando" y reaparezca en "pendientes"
                        self.refreshAbandonedWidget();
                        self.refreshRecoveredWidget();
                    } else {
                        alert("Error: No se pudo devolver la llamada. Puede que ya no estuviera en proceso.");
                        self.refreshAbandonedWidget();
                    }
                },
                error: function () {
                    alert("Error de comunicación al intentar devolver la llamada.");
                }
            });
        }
    }
...
generateAbandonedTableHtml: function (data) {
    ...
    data.forEach(call => {
        ...
        // =========================================================
        // === AÑADIDO: LÓGICA SIMPLE Y SEGURA PARA EL ICONO DE NOTA ===
        if (call.notes) {
            // Si call.notes no es null, undefined, o un string vacío, añade el icono.
            // No intentamos leer el contenido, solo comprobar si existe.
            info_block += ` <a href="#" class="btn-show-note" data-note-content="${encodeURIComponent(call.notes)}" title="Ver nota"><i class="fa fa-commenting-o" style="color: #f39c12; margin-left: 5px;"></i></a>`;
        }
        // =========================================================
...
recover: function (uniqueid, agentId) {
    ...
    $.ajax({
        // url: por defecto usa GET, que es lo que el PHP espera
        ...
        success: (response) => {
            if (response === 'ALREADY_TAKEN') {
                ...
                // Refrescamos para mostrar el estado correcto
                this.refreshAbandonedWidget();
                return;
            }
            
            // Si todo va bien, manipulamos el HTML para dar respuesta visual
            row.removeClass('danger warning').addClass('info');
            var actionCell = row.find('td').last();
            actionCell.html(
                `<button class="btn btn-primary btn-xs btn-finalize-abandoned" data-uniqueid="${uniqueid}" title="Finalizar gestión"><i class="fa fa-flag-checkered"></i> Finalizar</button>` +
                ` <button class="btn btn-warning btn-xs btn-requeue-abandoned" data-uniqueid="${uniqueid}" title="Devolver a la cola (sin respuesta)"><i class="fa fa-undo"></i> Devolver</button>`
            );
        },
        error: function(xhr, status, error) {
             // Un error más detallado
             alert("Error al tomar la llamada. Revisa la consola (F12) para más detalles.");
             console.error("Error en AJAX a recover.php:");
        ...
        finalize: function(uniqueid) {
            ...
            // 1. Primero, preguntamos quiénes somos
            $.ajax({
                url: 'plugins/abandonedcalls/whoami.php',
                success: function(agentData) {
                    var agentId = (agentData && agentData.agent) ? agentData.agent : 'unknown_finalize';

                    // 2. Ahora que tenemos el agente, finalizamos la llamada
                    $.ajax({
                        ...
                        data: { 
                            uniqueid: uniqueid, 
                            notes: note,
                            agent: agentId // Usamos el agente que acabamos de obtener
                        },
                        success: () => {
                            self.refreshAbandonedWidget();
                            self.refreshRecoveredWidget();
